// TEST: list size inferred through if-expression uses max branch size for forEach fan-out
// EXPECT_FINDINGS: 1
// EXPECT_MESSAGE_CONTAINS: @RequireBandwidth(40.0) is too small
...
@RequireBandwidth(minBandwidth = 40.0) // too small: max(5,10) * 5.0 = 50.0
suspend fun loadGallery(degraded: Boolean) = coroutineScope {
    val images =
        if (degraded) {
            listOf("a","b","c","d","e")
        } else {
